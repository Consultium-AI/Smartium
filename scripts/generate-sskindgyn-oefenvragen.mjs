#!/usr/bin/env node
/**
 * Genereer SSKindgyn oefenvragen uit de al gegenereerde samenvattingen
 * (.cursor/sskindgyn-cache/week-N/<slug>/raw.md), met de e-module
 * oefenvragen-masterprompt.
 *
 * Prompt: "tech -.../Smartium code-tech/smartium prompt oefenvragen e-module_vo.docx"
 * Output: src/questions/sskindgyn-weekN-<slug>/*.js (zelfde bestandsconventie als
 * de bestaande blok9/blok10 oefenvragen).
 *
 * Usage:
 *   node scripts/generate-sskindgyn-oefenvragen.mjs --list
 *   node scripts/generate-sskindgyn-oefenvragen.mjs --module antenatale-zorg
 *   node scripts/generate-sskindgyn-oefenvragen.mjs --limit 1
 *   node scripts/generate-sskindgyn-oefenvragen.mjs --all
 */
import fs from 'fs'
...
import { discoverModules, lmeIdForModule, moduleBasePascal, cacheRawMdPath } from './lib/sskindgynModules.mjs'
...
function buildUserPrompt(mod, sourceText) {
  return `De samenvatting van de e-module "${mod.displayTitle}" (${mod.weekLabel}, Kindergeneeskunde & Gynaecologie) is hieronder compleet. Maak nu de definitieve oefenvragen volgens je instructies.
...
async function processModule(mod, env, systemPrompt) {
  ...
  const bronComment = `SSKindgyn samenvatting — ${mod.weekLabel}, ${mod.name} (auto-gegenereerd)`
