#!/usr/bin/env node
/**
 * Layer 2: beoordeel/herschrijf/verwerp de layer-1 Blok 6 flashcard-drafts
 ...
 * Zelfde patroon als review-ssinterne-flashcards.mjs.
 *
 * Bron: .cursor/blok6-cache/week-N/casus-C/<slug>/flashcards-draft.json (layer 1)
 * Output: src/data/flashcards-blok6/<lmeId>.json + index.js
 *
 * Usage:
 *   node scripts/review-blok6-flashcards.mjs --module basis-ecg
 *   node scripts/review-blok6-flashcards.mjs --all --skip-existing
 */
import fs from 'fs'
...
const DATA_DIR = path.join(ROOT, 'src', 'data', 'flashcards-blok6')
...
function writeIndex() {
  ...
  const content = `// AUTO-GEGENEREERD door scripts/review-blok6-flashcards.mjs - niet handmatig bewerken.
${imports}

export const blok6FlashcardDecks = [
${arr}
]

export default blok6FlashcardDecks
`
...
async function processModule(mod, env, reviewerPrompt) {
  ...
  console.log(`\n=== ${mod.caseLabel} — ${mod.displayTitle} (${draftCards.length} draft-kaarten) ===`)
