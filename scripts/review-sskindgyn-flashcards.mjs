#!/usr/bin/env node
/**
 * Layer 2: beoordeel/herschrijf/verwerp de layer-1 SSKindgyn flashcard-drafts
 ...
 *
 * Bron: .cursor/sskindgyn-cache/week-N/<slug>/flashcards-draft.json (layer 1)
 * Output: src/data/flashcards-sskindgyn/<lmeId>.json + index.js
 *
 * Usage:
 *   node scripts/review-sskindgyn-flashcards.mjs --module antenatale-zorg
 *   node scripts/review-sskindgyn-flashcards.mjs --all --skip-existing
 */
import fs from 'fs'
...
import { discoverModules, lmeIdForModule, cacheRawMdPath, cacheFlashcardsDraftPath, BLOK_LABEL } from './lib/sskindgynModules.mjs'

const __dirname = path.dirname(fileURLToPath(import.meta.url))
const ROOT = path.resolve(__dirname, '..')
const OPENAI_URL = 'https://api.openai.com/v1/chat/completions'
const MAX_COMPLETION_TOKENS = 32000
const DATA_DIR = path.join(ROOT, 'src', 'data', 'flashcards-sskindgyn')
...
function writeIndex() {
  ...
  const content = `// AUTO-GEGENEREERD door scripts/review-sskindgyn-flashcards.mjs - niet handmatig bewerken.
${imports}

export const sskindgynFlashcardDecks = [
${arr}
]

export default sskindgynFlashcardDecks
`
