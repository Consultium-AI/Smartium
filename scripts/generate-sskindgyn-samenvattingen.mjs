#!/usr/bin/env node
/**
 * Genereer SSKindgyn (Kindergeneeskunde & Gynaecologie) samenvattingen uit
 * screenshots per e-module, in dezelfde look als blok 5/10.
 *
 * Pijplijn per module:
 ...
 *
 * Usage:
 *   node scripts/generate-sskindgyn-samenvattingen.mjs --list
 *   node scripts/generate-sskindgyn-samenvattingen.mjs --module antenatale-zorg
 *   node scripts/generate-sskindgyn-samenvattingen.mjs --limit 1
 *   node scripts/generate-sskindgyn-samenvattingen.mjs --all
 *   node scripts/generate-sskindgyn-samenvattingen.mjs --module antenatale-zorg --jsx-only
 */
import fs from 'fs'
...
import { emitSummaryComponent, emitRegistry } from './emit-sskindgyn-summary.mjs'
...
const CACHE_DIR = path.join(ROOT, '.cursor', 'sskindgyn-cache')
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      const res = await fetch(OPENAI_URL, {
        ...
        body: JSON.stringify({ model, messages, temperature: 0.35, max_completion_tokens: maxTokens }),
      })
...
async function extractNotesFromBatch({ apiKey, model, mod, batch, batchIndex, totalBatches }) {
  const content = [
    {
      type: 'text',
      text: `E-module "${mod.name}" (${mod.weekLabel}, ${BLOK_LABEL}) — batch ${batchIndex + 1}/${totalBatches}. Noteer ALLE feitelijke inhoud uit deze afbeeldingen (tekst, tabellen, leerdoelen, kernstof, klinische relevantie). Geen samenvatting, alleen gestructureerde notities in het Nederlands.`,
    },
...
async function generateFinalSummary({ apiKey, model, systemPrompt, mod, notes }) {
  const userText = `De upload is compleet. Hieronder staan alle notities uit de e-module "${mod.name}" (${mod.weekLabel}, ${BLOK_LABEL}), screenshots in volgorde. Maak nu één definitieve samenvatting volgens je instructies.\n\n${notes}`
  return callOpenAI({
    apiKey,
    model,
    messages: [
...
function deriveDescription(markdown) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // sla kopjes over (Leerdoelen, korte titelregels zonder eindpunt)
...
async function processModule(mod, env, systemPrompt) {
  ...
  // markdown -> structure -> sectie-JSX
  const markdown = fs.readFileSync(rawPath, 'utf8')
  const blocks = markdownToBlocks(markdown)
  fs.writeFileSync(structPath, JSON.stringify(blocks, null, 2), 'utf8')
  fs.mkdirSync(sectionsDir, { recursive: true })
  // Rijke gedeelde componenten (zelfde look als blok 5/10). Schrijf vóór generatie
  // zodat gen_structure_sections.py de minimale variant niet aanmaakt.
...
async function main() {
  ...
  if (listFlag) {
    console.log(`SSKindgyn modules: ${allModules.length}`)
