#!/usr/bin/env node
/**
 * Emit de container-component + registry voor Blok 6 samenvattingen.
 * Wordt aangeroepen door scripts/generate-blok6-samenvattingen.mjs.
 * Zelfde patroon als scripts/emit-sskindgyn-summary.mjs / emit-ssinterne-summary.mjs.
 */
import fs from 'fs'
...
export function emitSummaryComponent(mod, meta = {}) {
  ...
  const content = `// Auto-generated Blok 6 samenvatting — do not edit by hand.
// Regenerate: node scripts/generate-blok6-samenvattingen.mjs --module ${mod.slug}
import {
...
const ${compName} = () => (
  <SummaryLayout
    ...
    blokLabel=${JSON.stringify(BLOK_LABEL)}
...
export function emitRegistry() {
  ...
  for (const mod of discoverModules(ROOT)) {
    ...
    const rel = path.relative(path.join(ROOT, 'src', 'summaries'), compFile).split(path.sep).join('/')
    entries.push({ mod, compName, rel: `./${rel}` })
  }
  ...
  const registry = `// Auto-generated — scripts/emit-blok6-summary.mjs
${imports}

export const BLOK6_SUMMARIES = {
${map}
}
`
  fs.writeFileSync(
    path.join(ROOT, 'src', 'summaries', 'blok6SummariesRegistry.js'),
    registry,
    'utf8'
  )
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  if (arg === 'registry') {
    ...
    console.error('Usage: node scripts/emit-blok6-summary.mjs registry')
  }
}
