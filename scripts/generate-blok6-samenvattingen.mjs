#!/usr/bin/env node
/**
 * Genereer Blok 6 (BA2) samenvattingen uit screenshots per LME, in dezelfde
 * look als blok 5/9/10/SSKindgyn/SSInterne.
 *
 ...
 *
 * Bron: "SS1B6-smartium/week N/Casus C - <titel>/LME <titel>/"
 * Prompt: "tech -.../Smartium code-tech/Smartium prompt samenvatting emodule_vo.docx"
 *
 * Usage:
 *   node scripts/generate-blok6-samenvattingen.mjs --list
 *   node scripts/generate-blok6-samenvattingen.mjs --module basis-ecg
 *   node scripts/generate-blok6-samenvattingen.mjs --limit 1
 *   node scripts/generate-blok6-samenvattingen.mjs --all --skip-existing
 */
import fs from 'fs'
...
import {
  ...
  SECTION_SHARED,
  BLOK_LABEL,
} from './lib/blok6Modules.mjs'
import { emitSummaryComponent, emitRegistry } from './emit-blok6-summary.mjs'
...
const CACHE_DIR = path.join(ROOT, '.cursor', 'blok6-cache')
...
async function extractNotesFromBatch({ apiKey, model, mod, batch, batchIndex, totalBatches }) {
  const content = [
    {
      type: 'text',
      text: `E-module "${mod.displayTitle}" (${mod.weekLabel}, ${mod.casusName}, ${BLOK_LABEL}) — batch ${batchIndex + 1}/${totalBatches}. Noteer ALLE feitelijke inhoud uit deze afbeeldingen (tekst, tabellen, leerdoelen, kernstof, klinische relevantie). Geen samenvatting, alleen gestructureerde notities in het Nederlands.`,
    },
...
async function generateFinalSummary({ apiKey, model, systemPrompt, mod, notes }) {
  const userText = `De upload is compleet. Hieronder staan alle notities uit de e-module "${mod.displayTitle}" (${mod.weekLabel}, ${mod.casusName}, ${BLOK_LABEL}), screenshots in volgorde. Maak nu één definitieve samenvatting volgens je instructies.\n\n${notes}`
  return callOpenAI({
    apiKey,
    model,
    messages: [
...
async function processModule(mod, env, systemPrompt) {
  ...
  const cacheMod = path.join(CACHE_DIR, `week-${mod.weekNum}`, `casus-${mod.casusNum}`, mod.slug)
  ...
  if (!jsxOnly) {
    ...
    console.log(`\n=== ${mod.caseLabel} — ${mod.displayTitle} (${images.length} images, ${docs.length} pdf/docx) ===`)
