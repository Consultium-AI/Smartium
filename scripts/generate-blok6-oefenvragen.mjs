#!/usr/bin/env node
/**
 * Genereer Blok 6 oefenvragen uit de al gegenereerde samenvattingen
 * (.cursor/blok6-cache/week-N/casus-C/<slug>/raw.md), met de e-module
 * oefenvragen-masterprompt. Zelfde patroon als generate-ssinterne-oefenvragen.mjs.
 *
 * Prompt: "tech -.../Smartium code-tech/smartium prompt oefenvragen e-module_vo.docx"
 * Output: src/questions/blok6-weekN-casusC-<slug>/*.js (zelfde bestandsconventie als
 * de bestaande blok9/blok10/sskindgyn/ssinterne oefenvragen).
 *
 * Usage:
 *   node scripts/generate-blok6-oefenvragen.mjs --list
 *   node scripts/generate-blok6-oefenvragen.mjs --module basis-ecg
 *   node scripts/generate-blok6-oefenvragen.mjs --limit 1
 *   node scripts/generate-blok6-oefenvragen.mjs --all --skip-existing
 */
import fs from 'fs'
...
import { discoverModules, lmeIdForModule, moduleBasePascal, cacheRawMdPath, BLOK_LABEL } from './lib/blok6Modules.mjs'
...
function buildUserPrompt(mod, sourceText) {
  return `De samenvatting van de e-module "${mod.displayTitle}" (${mod.weekLabel}, ${mod.casusName}, ${BLOK_LABEL}) is hieronder compleet. Maak nu de definitieve oefenvragen volgens je instructies.
...
async function processModule(mod, env, systemPrompt) {
  ...
  const bronComment = `Blok 6 samenvatting — ${mod.caseLabel}, ${mod.displayTitle} (auto-gegenereerd)`
