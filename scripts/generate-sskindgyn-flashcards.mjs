#!/usr/bin/env node
/**
 * Genereer SSKindgyn Anki-flashcards (layer 1 / draft) uit de al gegenereerde
 * samenvattingen (.cursor/sskindgyn-cache/week-N/<slug>/raw.md), met de
 * SMARTIUM ANKI CARD GENERATOR (layer 1) masterprompt.
 *
 * Output: .cursor/sskindgyn-cache/week-N/<slug>/flashcards-draft.json
 * (layer 2 reviewer leest dit bestand en schrijft pas de publieke deck)
 *
 * Usage:
 *   node scripts/generate-sskindgyn-flashcards.mjs --list
 *   node scripts/generate-sskindgyn-flashcards.mjs --module antenatale-zorg
 *   node scripts/generate-sskindgyn-flashcards.mjs --all --skip-existing
 */
import fs from 'fs'
