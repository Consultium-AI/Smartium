// Week 6 Casus 12 LME 3 - Afbeelding 16: Swiss Cheese
export const image16SwissCheeseQuestions = [
  {
    id: 1,
    question: "Waarvoor wordt het Swiss Cheese Model gebruikt?",
    options: [
      { letter: "A", text: "Weergeven van systematische barrières – incident wanneer gaten op één lijn komen" },
      { letter: "B", text: "Analyseren van successen" },
      { letter: "C", text: "Voorspellen van het weer" },
      { letter: "D", text: "Medicatie bereiden" },
    ],
    correctAnswer: "A",
    category: "Swiss Cheese",
  },
  {
    id: 2,
    question: "Wie ontwikkelde het Swiss Cheese Model?",
    options: [
      { letter: "A", text: "James Reason" },
      { letter: "B", text: "Louis Pasteur" },
      { letter: "C", text: "Florence Nightingale" },
      { letter: "D", text: "Hippocrates" },
    ],
    correctAnswer: "A",
    category: "Swiss Cheese",
  },
]
