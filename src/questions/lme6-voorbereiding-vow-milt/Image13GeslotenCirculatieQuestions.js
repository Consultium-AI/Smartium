// Week 5 Casus 7 LME 6 - Afbeelding 13: Gesloten circulatie
export const image13GeslotenCirculatieQuestions = [
  {
    id: 1,
    question: "In welk van onderstaande structuren komt het bloed bij de gesloten bloedcirculatie NIET terecht?",
    options: [
      { letter: "A", text: "Rode pulpa parenchym" },
      { letter: "B", text: "Miltvene" },
      { letter: "C", text: "Veneuze sinus" },
      { letter: "D", text: "Penseelarteriole" },
    ],
    correctAnswer: "A",
    category: "Gesloten circulatie",
  },
  {
    id: 2,
    question: "Waarom stroomt het bloed sneller via de gesloten circulatie?",
    options: [
      { letter: "A", text: "Omdat het bloed binnen de vaten blijft en direct van arteriole naar veneuze sinus stroomt" },
      { letter: "B", text: "Omdat er minder bloed doorheen stroomt" },
      { letter: "C", text: "Omdat de vaten wijder zijn" },
      { letter: "D", text: "Omdat er geen filtratie plaatsvindt" },
    ],
    correctAnswer: "A",
    category: "Gesloten circulatie",
  },
]
