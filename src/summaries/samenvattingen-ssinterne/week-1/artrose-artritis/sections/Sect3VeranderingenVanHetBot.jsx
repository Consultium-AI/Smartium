import { Inline, PBody, DataTable, SubHeading, VraagjeBox } from './SectionShared'

export default function Section() {
  return (
    <div className="space-y-5 text-slate-700 dark:text-slate-300 leading-relaxed">
      <SubHeading level={2}><Inline>{`3. Veranderingen van het bot`}</Inline></SubHeading>
      <p className="leading-relaxed"><Inline>{`Let op:`}</Inline></p>
      <ul className="list-disc pl-6 space-y-2">
        <li><Inline>{`botdensiteit: normaal of osteopen?`}</Inline></li>
        <li><Inline>{`peri-articulaire osteopenie?`}</Inline></li>
        <li><Inline>{`subchondrale sclerose?`}</Inline></li>
        <li><Inline>{`erosies?`}</Inline></li>
        <li><Inline>{`subchondrale cysten?`}</Inline></li>
        <li><Inline>{`enthesitis / enthesopathie?`}</Inline></li>
      </ul>
    </div>
  )
}
