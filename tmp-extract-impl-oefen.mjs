...
const docx = path.join(
  root,
  'SVdocs/SS1B5-smartium/Week 7/Casus 14 - AI in de zorg/LMO Implementatie in de dermatologische praktijk oefenvragen.docx'
)
const zip = path.join(os.tmpdir(), 'impl-oefen.zip')
const tmp = path.join(os.tmpdir(), 'impl-oefen-unzip')
...
fs.writeFileSync(path.join(root, 'impl-oefen-paras.json'), JSON.stringify(paras, null, 2), 'utf8')
console.log('count', paras.length)
paras.forEach((p, i) => console.log(String(i).padStart(2, '0'), p.slice(0, 140)))
