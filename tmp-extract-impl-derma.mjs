...
const docx = path.join(
  root,
  'SVdocs/SS1B5-smartium/Week 7/Casus 14 - AI in de zorg/LMO Implementatie in de dermatologische praktijk samenvatting.docx'
)
const zip = path.join(os.tmpdir(), 'impl-derma.zip')
const tmp = path.join(os.tmpdir(), 'impl-derma-unzip')
...
fs.writeFileSync(path.join(root, 'impl-derma-paras.json'), JSON.stringify(paras, null, 2), 'utf8')
console.log('count', paras.length)
paras.forEach((p, i) => console.log(String(i).padStart(2, '0'), p.slice(0, 100)))
